...
def board_radii(r_d, cfg):
    ...
    r_ib = r_d * (
        cfg.board.r_inner_bull / cfg.board.r_double
    )  # inner bull radius, in px
    r_ob = r_d * (
        cfg.board.r_outer_bull / cfg.board.r_double
    )  # outer bull radius, in px
    w_dt = cfg.board.w_double_treble * (
        r_d / cfg.board.r_double
    )  # width of double and treble
...
def draw(img, xy, cfg, circles, score, color=(255, 255, 0)):
    ...
    for i, [x, y] in enumerate(xy):
        ...
        if i >= 4:
            ...
            cv2.circle(img, (x, y), 10, c, 1)
            cv2.putText(img, str(i + 1), (x + 8, y), font, font_scale / 2, c, line_type)
    return img
...
def get_bounding_box(img_path, scale=0.2):
    ...
    def on_click_bbox(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            if len(xy_bbox) < 2:
                xy_bbox.append(
                    [round((x / w) * img.shape[1]), round((y / h) * img.shape[0])]
                )
...
def main(cfg, folder, scale, draw_circles, dart_score=True):
    ...
    while i < len(imgs):
        ...
        cv2.putText(
            crop,
            "{}/{} {}".format(i + 1, len(annot), a["img_name"]),
            (0, 12),
        ...
        while True:
            ...
            if key == ord("b"):  # draw new bounding box
                ...
                annot.at[idx, "bbox"] = get_bounding_box(
                    osp.join(img_dir, a["img_name"]), scale
                )
                break
