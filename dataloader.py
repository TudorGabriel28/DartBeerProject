...
datasets = {
    ...
    "utrecht": {
        ...
        # "test": ["utrecht_12_22_2024_test_2"],
    },
...
def warp_perspective(img, xy, rho):
    ...
    M = cv2.getPerspectiveTransform(
        np.float32(four_points), np.float32(perturbed_four_points)
    )
...
def load_tfds(
    cfg, bbox_to_gt_func, split="train", return_xy=False, batch_size=32, debug=False
):
    ...
    if not return_xy:
        if cfg.model.tiny:
            ds = ds.map(
                lambda img, gt1, gt2: set_shapes_tiny(img, gt1, gt2, input_size),
                num_parallel_calls=AUTO,
            )
        else:
            ds = ds.map(
                lambda img, gt1, gt2, gt3: set_shapes(img, gt1, gt2, gt3, input_size),
                num_parallel_calls=AUTO,
            )
...
if __name__ == "__main__":
    ...
    ds = load_tfds(
        cfg, bbox_to_gt_func, split="train", return_xy=True, batch_size=1, debug=True
    )
